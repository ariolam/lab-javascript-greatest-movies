...
function dramaMoviesScore(moviesArray) {
    const dramaMovies = moviesArray.filter((movie) => movie.genre);

    const dramaScore = dramaMovies.reduce(
        (acc, dramaMovie) => acc + dramaMovie.score
    );
    let avgScore = dramaScore / dramaMovies.length;
    return Math.round(avgScore * 100) / 100;
}
